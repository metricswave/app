...
export default function TeamSettings() {
    ...
    return (
        <SectionContainer align={"left"} size={"big"}>
            <div className="flex flex-col space-y-6 max-w-content">
                ...
                <div className="flex flex-col gap-6 md:gap-10">
                    <div className="flex flex-col gap-2">
                        ...
                        <AnimateHeight id={`domain-save-button`} height={formChanged ? "auto" : 0} duration={300}>
                            <PrimaryButton
                                className="w-full"
                                text={"Save"}
                    ...
                    <div className="flex flex-col gap-10">
                        <div className="flex flex-col gap-2">
                            ...
                            <UserRow key={"owner"} user={team.owner} role={"owner"} teamId={team.id} />
        ...
        <>
            <div className="flex flex-col gap-4 border soft-border p-4">
                <div className="flex flex-col gap-1">
                    <p>
                        <strong>You can't not undo this action.</strong>
                    </p>
                ...
                <DeleteButton
                    onClick={async () => {
                        context.teamState.deleteTeam(teamId).then(() => {
                            context.userState.refreshUser(true);
                            navigate("/");
                        });
                    }}
        ...
        <div className="flex flex-row justify-between items-center p-3 rounded-sm border soft-border">
            <div className="flex flex-col items-start gap-1">
                <div className="flex flex-row gap-2 text-sm items-baseline">
                    <span>{user.name}</span>
                    <UserTag role={role} user={user} currentUser={currentUser} />
                </div>
...
function InviteRow({ invite, onRevoked }: { invite: TeamInvite; onRevoked?: () => void }) {
    return (
        <div className="flex flex-row justify-between items-center p-3 rounded-sm border soft-border text-sm">
            <div>
                {invite.email} <span className="opacity-50">(Invited)</span>
            </div>

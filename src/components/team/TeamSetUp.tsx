...
export function TeamSetUp({ onFinished }: Props) {
    ...
    useEffect(() => {
        const interval = setInterval(() => context.userState.refreshUser(true), FIVE_SECONDS * 1000);
    ...
    return (
        <SectionContainer>
            <div className="-mx-2.5 pb-64 grid gap-4 grid-cols-1 md:grid-cols-2 ">
                <div
                    className={twMerge(
                        "relative group bg-white dark:bg-zinc-800/40 rounded-sm p-5 pb-4 shadow col-span-2",
                    )}
                >
                    <PageTitle

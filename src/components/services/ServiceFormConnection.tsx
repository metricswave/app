...
export default function ServiceFormConnection({ channel, onCreated: created }: Props) {
    ...
    const [values, setValues] = useState<{ [key: string]: string }>(
        channel.configuration.form.fields.reduce(
            (acc, field) => {
                acc[field.name] = "";
                return acc;
            },
        ...
        channel.configuration.form.fields.forEach((field) => {
            ...
            if (field.validation !== undefined) {
                ...
                if (
                    ...
                    Number(values[field.name]) > field.validation.max_value
                ) {
                    errors[field.name] =
                        field.validation.max_value === 0
    ...
    return (
        <div>
            ...
            <div className="mt-8 mb-4">
                <Dialog.Title className="font-bold m-0 text-xl">{channel.configuration.form.title}</Dialog.Title>
            ...
            <div className="flex flex-col space-y-4">
                <div>
                    <LinkButton
                        href={`${channel.configuration.form.help.href}`}
                ...
                <PrimaryButton loading={loading} text="Create Service" onClick={handleSubmit} />
            </div>
        </div>
    );
}

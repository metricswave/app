    ...
    return (
        <div className="h-full flex flex-col justify-between">
            <div className="pb-6 flex flex-col sm:flex-row space-y-3 sm:space-y-0 items-start sm:items-center justify-between">
                {title !== undefined && <PageTitle title={title} description={`${average} average hits in period.`} />}
                {!hideViewSwitcher && (
            ...
            {stats.headers !== null && (
                <div className="flex flex-col sm:flex-row justify-start sm:items-center mb-10">
                    {Object.values(stats.headers).map((header, index) => {
                        const key = Object.keys(stats.headers!)[index] as "unique" | "pageViews" | "visits" | "total_income";
            ...
            <ResponsiveContainer width="100%" height={400}>
                <AreaChart data={data}>
                    ...
                    {compareWithPrevious && (
                        <Area type={"step"} dataKey="previous" stroke="#78716c" fill="url(#colorPrevious)" />
                    )}
                </AreaChart>
            </ResponsiveContainer>
        </div>
